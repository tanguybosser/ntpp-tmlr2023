...
def eval_metrics(pred, gold, probas, mask, results, n_class, multi_labels=False, test=False):
    if multi_labels:
    ...
    if multi_labels:
        ...
        if test:
            calibration, samples_per_bin = discrete_calibration(pred_labels, gold_labels, probas)
            results["calibration"] = calibration
            results["samples per bin"] = samples_per_bin
    return results

def discrete_calibration(pred, gold, proba, n_bins = 10):
    acc_per_bin, samples_per_bin = [], []
    for i in range(1,n_bins+1):
        ...
        if len(pred_bin) != 0:
            acc = (pred_bin == gold_bin).sum()/len(pred_bin)
        else:
            acc = 0
        acc_per_bin.append(acc)
    return acc_per_bin, samples_per_bin
