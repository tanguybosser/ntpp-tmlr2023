...
def take_3_by_3(x: th.Tensor, index:th.LongTensor) -> th.Tensor:
    b, l, d = x.shape

    batch_idx_shift = th.arange(start=0, end=b, device=x.device) * l * d
    batch_idx_shift = batch_idx_shift.reshape([b, 1, 1, 1])
    
    rep_idxs = th.arange(start=0, end=d, device=x.device).reshape([1, 1, 1, d])

    idxs_shift = batch_idx_shift + rep_idxs                      # [B,1, 1,D]

    idxs_shifted = index.unsqueeze(dim=-1) * d                   # [B,L,I,1]
    idxs_shifted = idxs_shifted + idxs_shift                     # [B,L,I,D]
    
    return th.take(x, index=idxs_shifted)
